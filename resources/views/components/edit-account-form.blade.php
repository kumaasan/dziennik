<form class="w-3/5 mx-auto bg-[#DCDCE2] rounded-lg shadow-lg px-8 pb-8 space-y-6" method="post" action="{{route('account.update')}}">
    ...
    <div class="mb-5">
        ...
        <input type="text" name="new_first_name" value="{{auth()->user()->first_name}}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" />
    </div>
    <div class="mb-5">
        <label class="block mb-2 text-sm font-medium text-gray-700">Nazwisko</label>
        <input type="text" name="new_last_name" value="{{auth()->user()->last_name}}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"/>
    </div>
    <div class="mb-5">
        <label class="block mb-2 text-sm font-medium text-gray-700">Email</label>
        <input type="email" name="new_email" value="{{auth()->user()->email}}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required />
    </div>
    <div class="mb-5">
        <label for="" class="block mb-2 text-sm font-medium text-gray-700">Minimalna średnia</label>
        <div class="flex items-center space-x-2">
            <input type="text" name="minimal_average" value="" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="np. 1,81" required />
        </div>
    </div>
